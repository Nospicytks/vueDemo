        ...
        <div class="center-title">
          <div class="center-title-left">
            <div class="center-title-left-item">
              ...
              <div class="center-title-left-item-right"><el-input v-model="coupon_name" placeholder="请输入"></el-input></div>
            </div>
            <div class="center-title-left-item">
          ...
          :header-cell-style="{background:'rgb(250,250,250)',color:'#606266'}"
             ...
             <el-table-column
              ...
              <template slot-scope="scope">
                <span v-if="scope.row.coupon_type === 2" style="color: #1890FF;">{{ scope.row.deduction_amount }}元</span>
                <span v-if="scope.row.coupon_type === 1" style="color: red;">{{ scope.row.deduction_time }}小时</span>
                <span v-if="scope.row.coupon_type === 3" style="color: red;">{{ scope.row.discount }}</span>
                <span v-if="scope.row.coupon_type === 4" style="color: #1890FF;">全免</span>
              </template>
             </el-table-column>
             <el-table-column
               label="状态"
             ...
             <el-table-column
               label="有效时间">
               <template slot-scope="scope">
                 <span>{{ scope.row.start_valid_period }}</span>
                 <span>~</span>
                 <span>{{ scope.row.end_valid_period }}</span>
               </template>
             </el-table-column>
             <el-table-column
               prop="parkingOpt"
               ...
               >
                <template slot-scope="scope">
                  <div class="tabopt">
                   <div @click="goDetail(scope.row.coupon_id)" >查看使用情况</div>
                   <div class="tabs"></div>
                   ...
                   <a-popconfirm
                       ...
                       ok-text="确定"
                       cancel-text="取消"
                       @confirm="deleteData(scope.row.coupon_id)"
                     >
                     <div>删除</div>
                   </a-popconfirm>
                  </div>
...
<script>
  import { couponList,couponListDelete } from '@/api/request'
  export default{
    data() {
      return {
        msg:['活动设置  /','优惠券管理 /','优惠券列表'],
        title:'优惠券列表',
        button:['批量导出','+ 新建'],
        titPath:'couponList',
        options:[
    ...
    watch:{
      currentPage:{
          handler(currentPage){
            this.getTabData(this.coupon_name,this.status,this.currentPage,this.pageSize)
          },
          deep: true,
          immediate: true,
        },
    ...
    methods:{
      ...
      // 修改
      update(data){
        let sdeduction_amount = 0;
        let discount_amount = 0;
        switch (data.coupon_type){
          case 1:
          sdeduction_amount = data.deduction_time;
          break;
          case 2:
          sdeduction_amount = data.deduction_amount;
          console.log(data.deduction_amount)
          break;
          case 3:
          sdeduction_amount = data.discount;
          discount_amount = data.discount_amount
          break;
          case 4:
          sdeduction_amount = '全免';
          break;
        }
        ...
        this.$router.push({
          ...
          query: {
            ...
            deduction_amount:sdeduction_amount,
            most_used:data.most_used,
            start_valid_period:data.start_valid_period,
            end_valid_period:data.end_valid_period,
            is_public:data.is_public,
            status:data.status,
            receive_valid_period:data.receive_valid_period,
            is_restricted_period:data.is_restricted_period,
            start_restricted_period:data.start_restricted_period,
            end_restricted_period:data.end_restricted_period,
      ...
      // 优惠卷列表
      getTabData(coupon_name,status,currentPage,pageSize){
        this.tabloading = true
        let tcoupon_name = coupon_name;
        let tstatus = status;
        let tcurrentPage = currentPage;
        let tpageSize = pageSize

        couponList(tcoupon_name,tstatus,tcurrentPage,tpageSize)
          .then(res => {
            if(res.data.code == 200){
