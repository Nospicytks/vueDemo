        ...
        <div class="center-title">
          <div class="center-title-left">
            <div class="center-title-left-item">
              <div class="center-title-left-item-left">使用人:</div>
              <div class="center-title-left-item-right"><el-input v-model="phone" placeholder="请输入"></el-input></div>
            </div>
            <div class="center-title-left-item">
              <div class="center-title-left-item-left">优惠券编号:</div>
              <div class="center-title-left-item-right"><el-input v-model="sn" placeholder="请输入"></el-input></div>
            </div>
            <div class="center-title-left-item">
          ...
          :header-cell-style="{background:'rgb(250,250,250)',color:'#606266'}"
             ...
             <el-table-column
               ...
               <template slot-scope="scope">
                 ...
                 <span v-if="scope.row.status == '3'" style="color: #00FA9A;">{{ scope.row.status_text }}</span>
               </template>
             </el-table-column>
            <el-table-column
              prop="end_valid_period"
              label="过期时间">
            </el-table-column>
           </el-table>
        </div>
...
<script>
  import { couponDetailList } from '@/api/request'
  export default{
    data() {
      return {
        msg:['活动设置  /','优惠券管理 /','优惠券列表 /','优惠券使用明细'],
        title:'优惠券使用明细',
        button:['批量导出','+ 新建'],
        titPath:'couponList',
        tabloading:false,
    ...
    watch:{
      currentPage:{
          handler(currentPage){
            this.getTabData(this.$route.query.coupon_id,this.phone,this.sn,this.status,this.currentPage,this.pageSize)
          },
          deep: true,
          immediate: true,
        },
    ...
    methods:{
      ...
      // 优惠卷使用情况列表
      getTabData(coupon_id,phone,sn,status,currentPage,pageSize){
        ...
        couponDetailList(tcoupon_id,tphone,tsn,tstatus,tcurrentPage,tpageSize)
          .then(res => {
