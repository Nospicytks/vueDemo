        ...
        <div class="center-title">
          <div class="center-title-left">
            <div class="center-title-left-item">
              <div class="center-title-left-item-left">操作人:</div>
            ...
            <div class="center-title-left-item">
              <div class="center-title-left-item-left">操作时间:</div>
             ...
             <el-table-column
               ...
               <template slot-scope="scope">
                  <div slot="reference" class="name-wrapper">
                    <el-tag v-for="(item,index) in scope.row.coupon_name" :key="index" size="medium" >
                       {{item}}
                    </el-tag>
                  </div>
                </template>
...
<script>
  import { manualPjList } from '@/api/request'
  export default{
    data() {
      return {
        msg:['活动设置  /','派券设置 /','手动派券记录'],
        title:'手动派券记录',
        button:['批量导出','+ 新建'],
        titPath:'manualPjList',
        tableData:[],
        nikename:'',
        phone:'',
        created_at:'',
    ...
    watch:{
      currentPage:{
          handler(currentPage){
            this.getTabData(this.nikename,this.phone,this.created_at,this.currentPage,this.pageSize)
          },
          deep: true,
          immediate: true,
        },
    ...
    methods:{
      ...
      goPages(index){
        switch (index){
          ...
          break;
          case 1:
          this.$router.push('/actConfig/paijuan/manualPjDo')
          break;
        }
      },
      // 重置
      ...
      // 手动派卷记录列表
      getTabData(nikename,phone,created_at,currentPage,pageSize){
        this.tabloading = true

        let tnikename = nikename;
        let tphone = phone;
        let tcreated_at = created_at;
        let tcurrentPage = currentPage;
        let tpageSize = pageSize

        manualPjList(tnikename,tphone,tcreated_at,tcurrentPage,tpageSize)
          .then(res => {
