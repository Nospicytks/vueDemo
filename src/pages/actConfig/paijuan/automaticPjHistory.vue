      ...
      <div class="center-title">
        <div class="center-title-left">
          <div class="center-title-left-item">
            <div class="center-title-left-item-left">用户电话:</div>
...
<script>
  import { deliverLogList } from '@/api/request'
  export default{
    data(){
      return {
        msg:['活动设置  /','自动派卷任务 /','自动派卷记录'],
        title:'自动派卷记录',
        button:['批量导出','取消'],
        titPath:'automaticPjHistory',
        phone:'',
        // 分页相关
        tabloading:false,
        currentPage:1,
    ...
    watch:{
      currentPage:{
          handler(currentPage){
            this.getTabData(this.$route.query.id,this.phone,this.currentPage,this.pageSize)
          },
          deep: true,
          immediate: true,
        },
    ...
    methods:{
      ...
      // 自动派卷任务列表
      getTabData(id,phone,currentPage,pageSize){
        ...
        deliverLogList(tid,tphone,tcurrentPage,tpageSize)
          .then(res => {
