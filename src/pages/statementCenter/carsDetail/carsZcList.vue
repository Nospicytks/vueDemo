             ...
             <template slot-scope="scope">
               <div class="tabopt">
                 <a-popconfirm
                     ...
                     ok-text="确定"
                     cancel-text="取消"
                     @confirm="leave(scope.row.order_id)"
                   >
                  <div>设为离场</div>
                  </a-popconfirm>
                  <div class="tabs"></div>
    ...
    <el-dialog title="车辆通行记录查询" @close="close" :visible.sync="dialogDetail"  :close-on-click-modal="false">
      <div class="diabox">
        ...
        <div class="diabox-right">
          ...
          <div class="diabox-right-item">
            <div>入场口：</div>
            <div>{{params.in_camera_title || '无'}}</div>
          </div>
          <div class="diabox-right-item">
            <div>进场时间：</div>
            <div>{{params.in_time || '无'}}</div>
          </div>
          <div class="diabox-right-item">
            <div>停车时长：</div>
            <div>{{params.total_time || '无'}}</div>
          </div>
          <div class="diabox-right-item">
            <div>车辆类型：</div>
          ...
          <div class="diabox-right-item">
            <div>车牌类型：</div>
            <div>{{params.car_subdivision_type || '无'}}</div>
          </div>
        </div>
      </div>
...
<script>
  import { carsZcList,childParking,setCarLeave } from '@/api/request'
  export default{
    data() {
      return {
    ...
    methods:{
      ...
      leave(id){
        let order_id = id
        setCarLeave(order_id)
          .then( res=> {
            if(res.data.code == 200){
              this.$message.success('离场成功!',3);
              this.getTabData(this.start_date,this.end_date,this.park_id,this.car_number,this.currentPage,this.pageSize)
            }else{
      ...
      getTabData(start_date,end_date,park_id,car_number,currentPage,pageSize){
        ...
        carsZcList(tstart_date,tend_date,tpark_id,tcar_number,tcurrentPage,tpageSize)
          .then(res => {
            if(res.data.code == 200){
